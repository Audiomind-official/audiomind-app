      ...
      <v-row class="fill-height" align-content="center" justify="center">
        <v-col v-bind:class="{ loading }" class="subtitle-1 text-center entries" cols="12">
          <v-card v-if="(total == 0) & (loading == false)" elevation="0">
            <v-card-text>
              <h1>Nenhum contato até agora</h1>Comece instalando o gravador no seu site para receber contatos!
            </v-card-text>
          </v-card>
          ...
          >
            <div class="pa-6 entry__info" style="display: flex;">
              ...
              <div style="line-height: 1;text-align: left;">
                ...
                <br />
                <small>{{ entry.property.name }}</small>
              </div>
            </div>
            <div class="pa-6">
    ...
    async getDataFromApi() {
      ...
      try {
        const entries = await this.$axios({
          url: `/properties/${this.property._id}/entries`,
          method: "GET",
          params: {
